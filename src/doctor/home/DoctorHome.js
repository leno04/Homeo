        ...
        <nav className="bg-[#AADCD2] p-4 flex justify-between items-center px-7 md:px-10 lg:px-20">
            <div className="text-black font-bold lg:text-2xl">LOGO</div>
                ...
                <button className="relative">
                    {/* notification bell */}
                    <svg className="w-8 h-8 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="#000000" viewBox="0 0 14 20">
                ...
                <div className="relative" ref={profileMenuRef}>
                    ...
                    {isMenuOpen && (
                        <div className="absolute right-0 mt-2 w-48 bg-[#d5ede9] rounded-lg shadow-lg py-2 z-50">
                            ...
                            <a href="#logout" className="block px-4 py-2 text-black">Log Out</a>
                        </div>
                    )}
                </div>
                ...
                {isOpen && (
                    <div ref={mobileMenuRef} className="md:hidden absolute w-48 top-16 right-4 rounded-md bg-[#d5ede9] p-4 flex flex-col items-start pl-5 space-y-4 z-50">
                        <button className="text-black" onClick={() => handleOptionChange('Option 2')}>Doctors</button>
                        <button className="text-black" onClick={() => handleOptionChange('Option 3')}>Appointments</button>
                        <button className="text-black" onClick={() => handleOptionChange('Option 4')}>Medical Records</button>
                        <button className="text-black" onClick={() => handleOptionChange('Option 5')}>Prescriptions</button>
                    </div>
                )} */}
            </div>
        </nav>
        <div>
            {selectedOption === 'Option 1' && <Schedules />}
            {selectedOption === 'Profile' && <DoctorProfile />}
        </div>
    </div>
  )
}
